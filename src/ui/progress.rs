...
pub fn view(ui: &mut egui::Ui, r: &mut RunningJob) {
    ...
    ui.vertical_centered(|ui| {
        ...
        // Rate + Abbrechen.
        ui.horizontal(|ui| {
            ui.add_space((ui.available_width() - bar_w).max(0.0) * 0.5);
            // §6.1: die *gemessene* Rate anzeigen.
            ui.label(
                egui::RichText::new(format!("gemessen: {:.0} MB/s", r.rate_mbps)).size(14.0),
            );
    ...
    // Ruhiges Log in einem eingerahmten, mitscrollenden Bereich.
    if !r.log.is_empty() {
        ui.add_space(4.0);
